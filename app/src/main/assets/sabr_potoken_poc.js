...
(function () {
    ...
    function report(result) {
        try {
            ...
            // Bridge not present (e.g. plain browser run): fall back to a redacted console result.
            try {
                console.log('[sabr-poc] result ok=' + !!result.ok
                    + ' tokenLength=' + (result.poTokenLength || 0)
                    + ' error=' + (result.error || ''));
            } catch (ignored) {
