 ...
 * lifetime).</p>
 */
public final class SabrWebViewPoTokenProvider implements SabrPoTokenProvider {
    ...
    public SabrWebViewPoTokenProvider(final Context context) {
    ...
    public byte[] getPoToken(final YoutubeSabrInfo info, final YoutubeSabrStreamState streamState,
        ...
        synchronized (mintLocks.computeIfAbsent(videoId, k -> new Object())) {
            ...
            final byte[] token;
            try {
                token = Base64.decode(tokenB64, Base64.URL_SAFE | Base64.NO_WRAP);
    ...
    private void waitForReadyThenInject(final WebView view,
        ...
        view.evaluateJavascript("document.readyState", value -> {
            ...
            if (complete || attempt >= READY_RETRIES) {
                ...
                mainHandler.postDelayed(
                        () -> waitForReadyThenInject(view, videoId, attempt + 1),
                        READY_POLL_MS);
            }
        });
    }
